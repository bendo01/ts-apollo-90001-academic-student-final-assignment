...
module.exports = {
    Query: {
        academic_student_final_assignment_transaction_schedule: async ( _:any, args:any, context: any ) => {
        ...
        academic_student_final_assignment_transaction_schedule_paginate: async ( _:any, args:any, context: any ) => {
            const check_auth_user_permission = new CheckUserPermission(context?.token, context?.user, context?.permission);
            let check_permission = await check_auth_user_permission.check();
            if (!check_permission) return null;
    ...
    Mutation: {
        create_academic_student_final_assignment_transaction_schedule: async ( _:any, input:any, context: any ) => {
            const check_auth_user_permission = new CheckUserPermission(context.token, context.user, context.permission);
            let check_permission = await check_auth_user_permission.check();
            if (!check_permission) return null;

            const data: object = {
                schedule_date: input.data.schedule_date,
                schedule_time: input.data.schedule_time,
                submission_id: input.data.submission_id,
            };
            return await AppDataSource.getRepository(ModelEntity).save(data);
        },
        update_academic_student_final_assignment_transaction_schedule: async ( _:any, input:any, context: any ) => {
            const check_auth_user_permission = new CheckUserPermission(context.token, context.user, context.permission);
            let check_permission = await check_auth_user_permission.check();
            if (!check_permission) return null;
        ...
        delete_academic_student_final_assignment_transaction_schedule: async ( _:any, input:any, context: any ) => {
            const check_auth_user_permission = new CheckUserPermission(context.token, context.user, context.permission);
            let check_permission = await check_auth_user_permission.check();
            if (!check_permission) return null;

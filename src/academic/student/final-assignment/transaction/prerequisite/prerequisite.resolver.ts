...
module.exports = {
    Query: {
        academic_student_final_assignment_transaction_prerequisite: async ( _:any, args:any, context: any ) => {
        ...
        academic_student_final_assignment_transaction_prerequisite_paginate: async ( _:any, args:any, context: any ) => {
            const check_auth_user_permission = new CheckUserPermission(context?.token, context?.user, context?.permission);
            let check_permission = await check_auth_user_permission.check();
            if (!check_permission) return null;
    ...
    Mutation: {
        create_academic_student_final_assignment_transaction_prerequisite: async ( _:any, input:any, context: any ) => {
            const check_auth_user_permission = new CheckUserPermission(context.token, context.user, context.permission);
            let check_permission = await check_auth_user_permission.check();
            if (!check_permission) return null;
        ...
        update_academic_student_final_assignment_transaction_prerequisite: async ( _:any, input:any, context: any ) => {
            const check_auth_user_permission = new CheckUserPermission(context.token, context.user, context.permission);
            let check_permission = await check_auth_user_permission.check();
            if (!check_permission) return null;
        ...
        delete_academic_student_final_assignment_transaction_prerequisite: async ( _:any, input:any, context: any ) => {
            const check_auth_user_permission = new CheckUserPermission(context.token, context.user, context.permission);
            let check_permission = await check_auth_user_permission.check();
            if (!check_permission) return null;

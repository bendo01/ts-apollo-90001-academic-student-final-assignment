...
module.exports = {
    Query: {
        academic_student_final_assignment_transaction_adviser: async ( _:any, args:any, context: any ) => {
        ...
        academic_student_final_assignment_transaction_adviser_paginate: async ( _:any, args:any, context: any ) => {
            const check_auth_user_permission = new CheckUserPermission(context?.token, context?.user, context?.permission);
            let check_permission = await check_auth_user_permission.check();
            if (!check_permission) return null;
    ...
    Mutation: {
        create_academic_student_final_assignment_transaction_adviser: async ( _:any, input:any, context: any ) => {
            const check_auth_user_permission = new CheckUserPermission(context.token, context.user, context.permission);
            let check_permission = await check_auth_user_permission.check();
            if (!check_permission) return null;
            ...
            const data: object = {
                ...
                lecturer_id: input.data.lecturer_id,
                submission_id: input.data.submission_id,
                adviser_category_id: input.data.adviser_category_id,
            };
            return await AppDataSource.getRepository(ModelEntity).save(data);
        },
        update_academic_student_final_assignment_transaction_adviser: async ( _:any, input:any, context: any ) => {
            const check_auth_user_permission = new CheckUserPermission(context.token, context.user, context.permission);
            let check_permission = await check_auth_user_permission.check();
            if (!check_permission) return null;
        ...
        delete_academic_student_final_assignment_transaction_adviser: async ( _:any, input:any, context: any ) => {
            const check_auth_user_permission = new CheckUserPermission(context.token, context.user, context.permission);
            let check_permission = await check_auth_user_permission.check();
            if (!check_permission) return null;
